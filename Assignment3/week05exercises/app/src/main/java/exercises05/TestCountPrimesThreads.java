...
// modified jst@itu.dk 2022-09-05
...
public class TestCountPrimesThreads {
  ...
  private static long countParallelNLocal(int range, int threadCount) {
    final int countsPerThread = range / threadCount;
    final AtomicLong counter = new AtomicLong();

    Thread[] threads = new Thread[threadCount];

    for (int threadNo = 0; threadNo < threadCount; threadNo++) {
      final int from = countsPerThread * threadNo;
      final int to = (threadNo + 1 == threadCount)
          ? range
          : countsPerThread * (threadNo + 1);
    ...
    return counter.get(); // change 0 to result;
  }
